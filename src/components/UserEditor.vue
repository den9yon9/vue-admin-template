    ...
    <div class="form">
      ...
      <div class="fields">
        <div class="input"><span class="input-title">姓名</span>
          <input type="text" placeholder="请输入police_name" v-model.trim="police_name">
        </div>
        <div class="input"><span class="input-title">警号</span>
          <input type="text" placeholder="请输入police_num" v-model.trim="police_num">
        </div>
        <div class="input"><span class="input-title">职位</span>
          <input type="text" placeholder="请输入title" v-model.trim="title">
        </div>
        <div class="input"><span class="input-title">密码</span>
          <input type="text" placeholder="请输入用户密码" v-model.trim="password">
        </div>
        <div class="input"><span class="input-title">职位</span>
          <input type="text" placeholder="请输入title" v-model.trim="title">
        </div>
        ...
        <div class="input"><span class="input-title">电话</span>
          <input type="text" placeholder="请输入电话" v-model.trim="title">
        </div>

        <div class="input"><span class="input-title">选项</span>
          <Selector placeholder="请选择password" :options="options" :value.sync="password" style="font-size: 1.25rem;flex-grow:1;"
            titlestyle="width:100%;"></Selector>
        </div>
  ...
  export default {
    ...
    methods: {
      async confirm() {
        ...
        if (!this.police_name || !this.police_num || !this.title || !this.password || this.time.length !== 2 || this.fileurl ===
          '../../static/avatar.png') {
        ...
        formData.append('police_name', this.police_name)
        formData.append('police_num', this.police_num)
    ...
    watch: {
      ...
      showeditor() {
        ...
        if (this.item) {
          this.police_name = this.item.police_name
          this.police_num = this.item.police_num
          this.title = this.item.police_num
